...
def loss_fn_reinforce(model, logprobs, rewards):
    # print(logprobs, rewards)
    # print(logprobs.shape, rewards.shape)
    loss = -logprobs@rewards
    # l2_loss = mx.sum(mx.array([mx.sum(param['weight'] ** 2).item() for param in model.parameters().values()])) * 1e-4
    return loss #+ l2_loss
...
if __name__ == '__main__':
    ...
    try:
        while running:
            ...
            if num_games%UPDATE_FREQUENCY == 0 and (reward == -1 or game_len>=MAX_GAME_LEN): #reward -1 just checks that the game is over
                ...
                for idx, rewards in enumerate(all_rewards):
                    ...
                    logprobs = all_logprobs[idx]
                    rewards = (rewards-mx.mean(rewards))/(mx.std(rewards)+1e-8) #(B,)
                    loss, grads = loss_and_grad_fn(model, logprobs, rewards)
                    losses.append(loss.item())
                    optimizer.update(model, grads) 
                    mx.eval(model.parameters(), optimizer.state)
                print(mx.mean(mx.array(losses)).item())
            ...
            if num_games%1000==1:
                model.save_weights(f'weights_{num_games}.safetensors')
